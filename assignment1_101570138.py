...
friend_names = [name for name, mins in workout_stats.items() if isinstance(mins, tuple)]
...
for i, friend in enumerate(friend_names):
    yoga_running = workout_list[i][0:2]
...
for i, friend in enumerate(friend_names[-2:]):
    weightlifting_minutes = workout_list[-2 + i][2]
    print(f"{friend}: {weightlifting_minutes}")
...
if search_name in friend_names:
